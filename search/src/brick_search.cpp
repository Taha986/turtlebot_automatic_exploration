...
void BrickSearch::imageCallback(const sensor_msgs::ImageConstPtr& image_msg_ptr)
{
  ...
  // INITIALISE VARIABLES FOR BLOB DETECTION
  ...
  cv::findContours(threshold_mat.clone(), contours, heirarchy, CV_RETR_TREE, CV_CHAIN_APPROX_NONE);

  //INTERATE THROUGH CONTOURS/BLOBS DETECTED AND MAKE A CRICLE SURROUNDING THE BLOB
...
//PRINT RED CIRCLE FOR VISUALISATION - dont need this might remove
count = centre.size();
...
/*
//PUBLISH IMAGE TO TOPIC - need to figure out proper way to publish image
...
void BrickSearch::mainLoop()
{
  ...
  //ros::NodeHandle nh;
  //ros::Subscriber sub = nh.subscribe("/camera/rgb/image_raw", 1000, imageCallback);
  ...
  while (ros::ok())
  {
   ROS_INFO("mainLoop");
    ...
    if (brick_found_ = 1) {
      ...
      ROS_INFO_STREAM("now sending stopping command" << brick_found_);
    ...
    // SET action_goal WITH GOAL POSITION
    ...
    gx = static_cast<int>(std::floor((pose_2d.x - map_.info.origin.position.x) / map_.info.resolution));//get grid position
    gy = static_cast<int>(std::floor((pose_2d.y - map_.info.origin.position.y) / map_.info.resolution));//get grid position
    ID = (gy-10) * static_cast<int>(map_.info.width) + gx;
    D = map_.data[ID];
    while(D == 0 && brick_found_ == 0)
    {
      pose_2d.y = pose_2d.y - map_.info.resolution;
      move_base_msgs::MoveBaseActionGoal action_goal{};
      ...
      gx = static_cast<int>(std::floor((pose_2d.x - map_.info.origin.position.x) / map_.info.resolution));//get grid position
      gy = static_cast<int>(std::floor((pose_2d.y - map_.info.origin.position.y) / map_.info.resolution));//get grid position
      ROS_INFO_STREAM("gridx: " << gx);
      ROS_INFO_STREAM("gridy: " << gy);
      ID = (gy-10) * static_cast<int>(map_.info.width) + gx;
      D = map_.data[ID];
